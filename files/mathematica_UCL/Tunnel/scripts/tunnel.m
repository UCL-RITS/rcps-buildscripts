...
(* Modified for UCL Legion and Grace - Jan 2017 *)
...
(* We need to fix the paths for the instllation in /shared/ucl/apps on Legion and Grace. *)
(* Should really be using $InstallationDirectory *)
...
VersionedKernelPath[system_String, versionNumber_] :=
	Which[
		...
		versionNumber >= 10.0,
			$InstallationDirectory <>  "/Executables/WolframKernel",
		True, (* default to Unix conventions *)
			$InstallationDirectory <> "/Executables/MathKernel"
	]
...
DefaultKernelPath[system_String, versionNumber_] :=
	Which[
		...
		system === "Windows",
			"MathKernel.exe",
		versionNumber >= 10.0,
			$InstallationDirectory <> "/bin/WolframKernel",
		True, (* default to Unix conventions *)
			$InstallationDirectory <> "/bin/MathKernel"
	]
